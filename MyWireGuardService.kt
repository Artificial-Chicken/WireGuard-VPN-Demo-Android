        ...
        return Config.Builder()
            .setInterface(
                com.wireguard.config.Interface.Builder()
                    .parsePrivateKey("PRIVATE_KEY") // Replace with a real private key
                    .addAddress(InetNetwork.parse("ADRESS"))
                    .addDnsServer(java.net.InetAddress.getByName("8.8.8.8"))
                    .build()
            )
            .addPeer(
                Peer.Builder()
                    .parsePublicKey("PUBLIC_KEY") // Replace with the server's public key
                    .setEndpoint(InetEndpoint.parse("END_POINT")) // Replace with your server IP/host and port
